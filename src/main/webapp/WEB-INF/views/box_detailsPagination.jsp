...
<div class="pagination">
    <div class="pagInfo"><fmt:message key="results"/>: ${albums.totalElements}</div>
    <div class="pagButtons">
        <c:choose>
            ...
            <c:otherwise>
                <a href="<c:url value="box_details?id=${box.id}&page=0&size=${albums.pageable.pageSize}"/>"
                   class="pagButton"
                   style="font-size: small; margin-top: 2px" type="button"><fmt:message key="first"/></a>
                <a href="<c:url value="box_details?id=${box.id}&page=${albums.number-1}&size=${albums.pageable.pageSize}"/>"
                   class="pagButton" type="button"> << </a>
            </c:otherwise>
        </c:choose>

        <div class="pageInfo"><fmt:message key="page"/> ${albums.number+1} <fmt:message key="of"/> ${albums.totalPages}</div>
        ...
        <c:choose>
            ...
            <c:otherwise>
                <a href="<c:url value="box_details?id=${box.id}&page=${albums.number+1}&size=${albums.pageable.pageSize}"/>"
                   class="pagButton" type="button"> >> </a>
                <a href="<c:url value="box_details?id=${box.id}&page=${albums.totalPages-1}&size=${albums.pageable.pageSize}"/>"
